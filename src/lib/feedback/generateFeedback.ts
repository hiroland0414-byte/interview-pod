...
export function buildFeedback(
  ...
  const coreOkCount =
    ...
    answers.filter((a, i) => {
      const q = questions[i];
      return q?.core ? (a || "").length >= 160 : false;
    }).length;
  ...
  const good = [
    ...
    coreOkCount > 0
      ? `コアとなる三大質問（志望動機・自己PR・学生時代に力を入れたこと）のうち、${coreOkCount}問では160文字以上の深さが出ており、エピソードの背景や気持ちまで伝えようとしている点は、とても良い傾向です。`
  ...
  if (avgChars < 160) {
    improveParts.push(
      `一方で、回答の平均文字数がやや少なく、面接官側から見ると「もう一歩踏み込んで聞いてみたい」と感じる場面が多くなりそうです。特にコアの三大質問では、推奨は200文字以上は使って、「きっかけ → 具体的なエピソード → そこから学んだこと・今後どう活かすか」という流れを意識できると、説得力が一気に増します。`
    );
  } else {
    improveParts.push(
